...
<body>
	...
	<div class="title">
		<div class="am-g am-g-fixed">
			<div class="am-u-sm-12">
				<h1 class="am-text-primary">Congcong's ChatRoom</h1>
			</div>
		</div>
	</div>
	<!-- title end -->
	...
	<script>
		$(function() {
			// initiate the message input box
			var um = UM.getEditor('myEditor');
			// focus the nickname 
			$('#nickname')[0].focus();
			// initiate a new websocket object, correlate with @ServerEndpoint("/websocket") at the server side
			...
			$('#send').on('click', function() {
				...
				if (!um.hasContents()) {	// check the input box is empty or not
					// get the focus of input box
					um.focus();
					// add trembling effect
					$('.edui-container').addClass('am-animation-shake');
					setTimeout("$('.edui-container').removeClass('am-animation-shake')", 1000);
				} else if (nickname == '') {	//  check the nickname is empty or not
					//focus the nickname input box
					$('#nickname')[0].focus();
					// add trembling effect
			...
			// add the message into the discussion content
